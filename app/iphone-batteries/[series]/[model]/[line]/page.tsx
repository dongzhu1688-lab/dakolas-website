...
import { getIphoneModelProduct, iphoneBatteryModelPages } from "@/lib/seo-expansion";
...
export function generateStaticParams() {
  return iphoneBatteryModelPages.map((product) => ({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const product = getIphoneModelProduct("iphone-batteries", series, model, line);
  if (!product) return {};
...
export default async function IPhoneBatteryModelLinePage({ params }: Props) {
  const { series, model, line } = await params;
  const product = getIphoneModelProduct("iphone-batteries", series, model, line);
  if (!product) notFound();
